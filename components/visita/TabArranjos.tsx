                    ...
                    {config.pastoreios.map((pastoreio: any, idx: number) => (
                        <div key={pastoreio.id} className="p-4 border border-slate-200 dark:border-slate-700 rounded-lg space-y-3 relative">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                                <select value={pastoreio.membro_id || ''} onChange={e => { const p = [...config.pastoreios]; p[idx] = { ...p[idx], membro_id: e.target.value }; updateConfig('pastoreios', p) }} className="p-2 text-sm border rounded-lg dark:bg-slate-800">
                                    <option value="">Irmão(ã) a visitar...</option>
                                    {membros.map(m => <option key={m.id} value={m.id}>{m.nome_completo}</option>)}
                                </select>
                                <select value={pastoreio.anciao_id || ''} onChange={e => { const p = [...config.pastoreios]; p[idx] = { ...p[idx], anciao_id: e.target.value }; updateConfig('pastoreios', p) }} className="p-2 text-sm border rounded-lg dark:bg-slate-800">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                                <input type="date" value={pastoreio.data || ''} onChange={e => { const p = [...config.pastoreios]; p[idx] = { ...p[idx], data: e.target.value }; updateConfig('pastoreios', p) }} className="p-2 text-sm border rounded-lg dark:bg-slate-800" />
                                <input type="time" value={pastoreio.hora || ''} onChange={e => { const p = [...config.pastoreios]; p[idx] = { ...p[idx], hora: e.target.value }; updateConfig('pastoreios', p) }} className="p-2 text-sm border rounded-lg dark:bg-slate-800" />
                            </div>
                            <textarea placeholder="Breve descrição do contexto (Ex: Inativo, doente, precisando encorajamento...)" value={pastoreio.memo || ''} onChange={e => { const p = [...config.pastoreios]; p[idx] = { ...p[idx], memo: e.target.value }; updateConfig('pastoreios', p) }} className="w-full p-2 text-sm border rounded-lg dark:bg-slate-800 h-20" />
                        </div>
                    ))}
