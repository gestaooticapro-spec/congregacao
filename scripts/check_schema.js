...
async function checkSchema() {
    console.log('Checking schema for programacao_semanal...');

    // Try to select the new columns
    const { data, error } = await supabase
        .from('programacao_semanal')
        .select('presidente_id, oracao_inicial_id, oracao_final_id')
        .limit(1);

    if (error) {
        console.error('Error selecting columns:', error.message);
        if (error.message.includes('does not exist')) {
            console.log('Migration 008 likely NOT applied.');
        }
    } else {
        console.log('Columns exist! Migration 008 likely applied.');
    }
}
