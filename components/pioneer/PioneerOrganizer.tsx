...
import { Check, ChevronLeft, ChevronRight, MapPin, MessageCircle, Navigation, Pause, Play, Plus, RefreshCw, Send, Trash2, Users, X } from 'lucide-react'
...
export default function PioneerOrganizer({ initialMode }: { initialMode: 'calendar' | 'revisits' }) {
...
function Tab({ active, onClick, children }: { active: boolean; onClick: () => void; children: React.ReactNode }) { return <button onClick={onClick} className={`rounded-xl py-2 text-sm font-semibold ${active ? 'bg-white text-slate-900 shadow dark:bg-slate-700 dark:text-white' : 'text-slate-500'}`}>{children}</button> }
...
function Studies({ activities, occurrences, tab, onTab, onEdit, onMaps, onDone, onSkip, onSchedule, onPause, onResume, onEnd, onDelete }: any) { const active = activities.filter((activity: Activity) => activity.tipo === 'ESTUDO' && ['ATIVA', 'PAUSADA'].includes(activity.status)); const ended = activities.filter((activity: Activity) => activity.tipo === 'ESTUDO' && activity.status === 'CANCELADA'); const names = new Map<string, string>(activities.filter((activity: Activity) => activity.tipo === 'ESTUDO').map((activity: Activity) => [String(activity.id), String(activity.pessoa_nome || 'Estudo')] as [string, string])); return <section><div className="mb-3 grid grid-cols-3 rounded-2xl bg-slate-200 p-1 dark:bg-slate-800"><Tab active={tab === 'active'} onClick={() => onTab('active')}>Ativos</Tab><Tab active={tab === 'history'} onClick={() => onTab('history')}>Histórico</Tab><Tab active={tab === 'ended'} onClick={() => onTab('ended')}>Encerrados</Tab></div>{tab === 'history' ? <div className="space-y-3">{occurrences.slice().sort((a: Activity, b: Activity) => b.created_at.localeCompare(a.created_at)).map((item: Activity) => <article key={item.id} className="rounded-2xl border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-900"><b className="text-slate-900 dark:text-white">{names.get(String(item.atividade_id)) || 'Estudo'}</b><p className="mt-1 text-sm text-slate-500">{item.status === 'REALIZADO' ? 'Realizado' : item.status === 'PULADO' ? 'Pulado' : `Remarcado para ${fmtDate(item.data_agendada)}`} • {fmtDate(item.data_original)}</p></article>)}{!occurrences.length && <Empty text="Nenhuma ocorrência registrada ainda." action="" />}</div> : <div className="space-y-3">{(tab === 'active' ? active : ended).map((study: Activity) => <StudyCard key={study.id} study={study} occurrences={occurrences} onEdit={() => onEdit(study)} onMaps={() => onMaps(study)} onDone={() => onDone(study)} onSkip={() => onSkip(study)} onReschedule={() => onSchedule(study)} onPause={() => onPause(study)} onResume={() => onResume(study)} onEnd={() => onEnd(study)} onDelete={() => onDelete(study)} />)}{!(tab === 'active' ? active : ended).length && <Empty text={tab === 'active' ? 'Nenhum estudo ativo.' : 'Nenhum estudo encerrado.'} action="" />}</div>}</section> }

function ActivityCard({ activity, nearby, onEdit, onMaps, onComplete, onSchedule, onTransfer, onCancelTransfer, onDelete }: any) { const revisit = activity.tipo === 'REVISITA'; return <article className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900"><div className="flex justify-between gap-2"><div><p className="text-xs font-bold uppercase tracking-wide text-emerald-600">{revisit ? 'Revisita' : activity.titulo || 'Planejamento'}</p><h3 className="font-bold text-slate-900 dark:text-white">{activity.pessoa_nome || activity.titulo || 'Atividade'}</h3><p className="mt-1 text-sm text-slate-500">{fmtDate(activity.__displayDate || activity.data_agendada)} {activity.hora_agendada ? `• ${String(activity.hora_agendada).slice(0, 5)}` : ''} {activity.tipo_agendamento ? `• ${activity.tipo_agendamento === 'COMBINADO' ? 'Combinado' : 'Planejado'}` : ''}</p></div>{activity.status === 'CONCLUIDA' ? <Badge color="emerald">Concluída</Badge> : activity.status === 'AGUARDANDO_TRANSFERENCIA' ? <Badge color="violet">Aguardando resposta</Badge> : isOverdue(activity) ? <Badge color="rose">Atrasada</Badge> : null}</div>{activity.observacoes && <p className="mt-3 line-clamp-2 text-sm text-slate-600 dark:text-slate-300">{activity.observacoes}</p>}{nearby.map((item: any) => <p key={item.other.id} className="mt-2 rounded-lg bg-amber-50 p-2 text-xs text-amber-800">Há outra atividade a {Math.round(item.km * 1000)} m neste dia: {item.other.pessoa_nome || item.other.titulo}.</p>)}<div className="mt-3 flex flex-wrap gap-2"><SmallButton onClick={onEdit}>Ver / editar</SmallButton>{(activity.latitude != null || activity.endereco) && <SmallButton onClick={onMaps}><MapPin className="mr-1 inline h-4 w-4"/>Mapa</SmallButton>}{activity.telefone && <a href={phoneUrl(activity.telefone)} target="_blank" rel="noreferrer" className="rounded-lg border border-emerald-200 px-3 py-1.5 text-sm text-emerald-700"><MessageCircle className="mr-1 inline h-4 w-4"/>WhatsApp</a>}{revisit && activity.status === 'ATIVA' && <><SmallButton primary onClick={onComplete}><Check className="mr-1 inline h-4 w-4"/>Concluir</SmallButton>{isOverdue(activity) && <SmallButton onClick={onSchedule}>Remarcar</SmallButton>}<SmallButton onClick={onTransfer}><Send className="mr-1 inline h-4 w-4"/>Passar</SmallButton></>}{revisit && activity.status === 'AGUARDANDO_TRANSFERENCIA' && <SmallButton onClick={onCancelTransfer}>Cancelar transferência</SmallButton>}{revisit && activity.status === 'CONCLUIDA' && <SmallButton primary onClick={onSchedule}><RefreshCw className="mr-1 inline h-4 w-4"/>Revisitar de novo</SmallButton>}<SmallButton danger onClick={onDelete}><Trash2 className="h-4 w-4"/></SmallButton></div></article> }

function StudyCard({ study, occurrences, onEdit, onMaps, onDone, onSkip, onReschedule, onPause, onResume, onEnd, onDelete }: any) { const next = study.status === 'ATIVA' ? nextStudy(study, occurrences) : null; return <article className="rounded-2xl border border-blue-200 bg-white p-4 shadow-sm dark:border-blue-900 dark:bg-slate-900"><div className="flex justify-between gap-2"><div><p className="text-xs font-bold uppercase tracking-wide text-blue-600">Estudo semanal</p><h3 className="font-bold text-slate-900 dark:text-white">{study.pessoa_nome}</h3>{next && <p className="mt-1 text-sm text-slate-500">Próximo: {fmtDate(next.scheduled)} {study.hora_agendada ? `• ${String(study.hora_agendada).slice(0, 5)}` : ''}</p>}</div><Badge color={study.status === 'PAUSADA' ? 'amber' : study.status === 'CANCELADA' ? 'slate' : 'blue'}>{study.status === 'PAUSADA' ? 'Pausado' : study.status === 'CANCELADA' ? 'Encerrado' : 'Toda semana'}</Badge></div>{study.observacoes && <p className="mt-3 line-clamp-2 text-sm text-slate-600 dark:text-slate-300">{study.observacoes}</p>}<div className="mt-3 flex flex-wrap gap-2"><SmallButton onClick={onEdit}>Ver / editar</SmallButton>{(study.latitude != null || study.endereco) && <SmallButton onClick={onMaps}><MapPin className="mr-1 inline h-4 w-4"/>Mapa</SmallButton>}{study.telefone && <a href={phoneUrl(study.telefone)} target="_blank" rel="noreferrer" className="rounded-lg border border-emerald-200 px-3 py-1.5 text-sm text-emerald-700"><MessageCircle className="mr-1 inline h-4 w-4"/>WhatsApp</a>}{study.status === 'ATIVA' && <><SmallButton primary onClick={onDone}><Check className="mr-1 inline h-4 w-4"/>Realizado</SmallButton><SmallButton onClick={onSkip}>Pular</SmallButton><SmallButton onClick={onReschedule}>Remarcar</SmallButton><SmallButton onClick={onPause}><Pause className="mr-1 inline h-4 w-4"/>Pausar</SmallButton><SmallButton onClick={onEnd}>Encerrar</SmallButton></>}{study.status === 'PAUSADA' && <><SmallButton primary onClick={onResume}><Play className="mr-1 inline h-4 w-4"/>Retomar</SmallButton><SmallButton onClick={onEnd}>Encerrar</SmallButton></>}<SmallButton danger onClick={onDelete}><Trash2 className="h-4 w-4"/></SmallButton></div></article> }
...
function SmallButton({ children, onClick, primary, danger }: any) { return <button onClick={onClick} className={`rounded-lg border px-3 py-1.5 text-sm ${primary ? 'border-emerald-600 bg-emerald-600 text-white' : danger ? 'border-rose-200 text-rose-700' : ''}`}>{children}</button> }
