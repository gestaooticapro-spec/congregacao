...
interface Assignment { funcao: string; membro: { nome_completo: string; nome_civil: string | null } | null }
    ...
    useEffect(() => {
        const carregar = async () => {
            ...
            const [{ data: apoios }, { data: programacao }] = await Promise.all([
                supabase.from('designacoes_suporte').select('funcao, membro:membros(nome_completo, nome_civil)').eq('data', proximaData),
                supabase.from('programacao_semanal').select('presidente_id').eq('data_reuniao', proximaData).maybeSingle(),
            ])
            setAssignments((apoios || []) as Assignment[])
            setIsPresidenteLogado(await isLogadoPresidenteMeioSemana(programacao?.presidente_id))
        }
        void carregar()
    }, [])
