                    ...
                    <table className="w-full text-left border-collapse min-w-full">
                        <thead>
                            <tr className="bg-slate-50 dark:bg-slate-800/50 text-slate-500 dark:text-slate-400 text-sm border-b border-slate-100 dark:border-slate-800">
                                <th className="py-3 px-2 sm:px-4 font-bold">Dia</th>
                                <th className="py-3 px-2 sm:px-4 font-bold">Hora</th>
                                <th className="py-3 px-2 sm:px-4 font-bold">Local</th>
                                <th className="py-3 px-2 sm:px-4 font-bold text-center">Obs</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                            {schedule.map((item, index) => (
                                <tr key={item.id || index} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                                    <td className="py-3 px-2 sm:px-4 text-slate-900 dark:text-white font-medium">{item.dia}</td>
                                    <td className="py-3 px-2 sm:px-4 text-slate-700 dark:text-slate-300">{item.hora}</td>
                                    <td className="py-3 px-2 sm:px-4">
                                        <span className={`inline-flex items-center px-1.5 sm:px-2 py-0.5 sm:py-1 rounded-full text-xs font-medium ${item.local.toLowerCase() === 'zoom'
                                    ...
                                    <td className="py-3 px-2 sm:px-4 text-center">
                                        {item.obs && item.obs.trim() !== '' && (
                                            <button
                                                ...
                                                className="text-slate-400 hover:text-blue-500 transition-colors inline-flex align-middle"
