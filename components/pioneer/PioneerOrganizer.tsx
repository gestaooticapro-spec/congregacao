...
function StudyCard({ study, occurrences, notes = [], onNote, onEdit, onMaps, onDone, onSkip, onReschedule, onPause, onResume, onEnd, onDelete }: any) { const next = study.status === 'ATIVA' ? nextStudy(study, occurrences) : null; const noteDate = study.__occurrenceOriginal || next?.original; const lastNote = next ? notes.find((item: Activity) => item.atividade_id === study.id && item.detalhes?.data_original < next.original) : null; return <article className="rounded-2xl border border-blue-200 bg-white p-4 shadow-sm dark:border-blue-900 dark:bg-slate-900"><div className="flex justify-between gap-2"><div><p className="text-xs font-bold uppercase tracking-wide text-blue-600">Estudo semanal</p><h3 className="font-bold text-slate-900 dark:text-white">{study.pessoa_nome}</h3>{next && <p className="mt-1 text-sm text-slate-500">Próximo: {fmtDate(next.scheduled)} {study.hora_agendada ? `• ${String(study.hora_agendada).slice(0, 5)}` : ''}</p>}</div><Badge color={study.status === 'PAUSADA' ? 'amber' : study.status === 'CANCELADA' ? 'slate' : 'blue'}>{study.status === 'PAUSADA' ? 'Pausado' : study.status === 'CANCELADA' ? 'Encerrado' : 'Toda semana'}</Badge></div>{lastNote?.detalhes?.nota && <div className="mt-3 rounded-xl bg-amber-50 p-3 text-sm text-amber-900 dark:bg-amber-950/30 dark:text-amber-100"><b className="block text-xs uppercase tracking-wide">Anotação da última visita</b><p className="mt-1">{lastNote.detalhes.nota}</p></div>}{study.observacoes && <p className="mt-3 line-clamp-2 text-sm text-slate-600 dark:text-slate-300">{study.observacoes}</p>}<div className="mt-3 flex flex-wrap gap-2"><SmallButton onClick={onEdit}>Ver / editar</SmallButton>{(study.latitude != null || study.endereco) && <SmallButton onClick={onMaps}><MapPin className="mr-1 inline h-4 w-4"/>Mapa</SmallButton>}{study.telefone && <a href={phoneUrl(study.telefone)} target="_blank" rel="noreferrer" className="rounded-lg border border-slate-300 px-3 py-1.5 text-sm text-slate-700 dark:border-slate-600 dark:text-slate-200"><MessageCircle className="mr-1 inline h-4 w-4"/>WhatsApp</a>}{study.status === 'ATIVA' && <><SmallButton onClick={() => onNote(study, noteDate)}><StickyNote className="mr-1 inline h-4 w-4"/>Obs. de hoje</SmallButton><SmallButton onClick={onDone}><Check className="mr-1 inline h-4 w-4"/>Realizado</SmallButton><SmallButton onClick={onSkip}>Pular</SmallButton><SmallButton onClick={onReschedule}>Remarcar</SmallButton><SmallButton onClick={onPause}><Pause className="mr-1 inline h-4 w-4"/>Pausar</SmallButton><SmallButton onClick={onEnd}>Encerrar</SmallButton></>}{study.status === 'PAUSADA' && <><SmallButton onClick={onResume}><Play className="mr-1 inline h-4 w-4"/>Retomar</SmallButton><SmallButton onClick={onEnd}>Encerrar</SmallButton></>}<SmallButton onClick={onDelete}><Trash2 className="h-4 w-4"/></SmallButton></div></article> }
